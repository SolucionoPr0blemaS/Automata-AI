...
class SchedulerApp(ctk.CTk):
    def __init__(self):
        ...
        # ====== COLUMNA IZQUIERDA ======
        self.lbl_real_time_title = ctk.CTkLabel(self.frame, text="           Real Time", font=("Arial", 14, "bold"), text_color="#FFFFFF")
        ...
        self.cb4 = ctk.CTkCheckBox(self.checkbox_frame, text="Sound", variable=self.var_sound, font=cb_font)
        self.cb4.grid(row=3, column=0, sticky="w", pady=(10, 0))

        # ====== COLUMNA DERECHA ======
        self.label = ctk.CTkLabel(self.frame, text="Execution time (HH:MM:SS):", font=("Arial", 14, "bold"), text_color="#FFFFFF")
        ...
        ctk.CTkLabel(self.frame, text="Repeat every (HHH:MM:SS):  ", font=("Arial", 12)).grid(row=3, column=1, sticky="e", padx=(0, 20), pady=(5, 0))
        ...
        self.btn_start = ctk.CTkButton(self.frame, text="Schedule", fg_color="#27ae60", width=160 , height=40, font=("Arial", 22, "bold"), command=self.toggle_scheduling)
    ...
    def toggle_scheduling(self):
        ...
        if len(target_time) != 8 or target_time.count(":") != 2:
            self.label.configure(text="Incorrect format. Use HH:MM:SS", text_color="#ff4444")
            return
        ...
        self.first_execution_done = False
        set_keep_awake(True)
        self.btn_start.configure(text="Update", fg_color="#d35400")
        
        self.label.configure(text="Waiting for execution...", text_color="#FFFFFF")
    ...
    def wait_and_execute(self):
        ...
        if self.running:
            ...
            self.running = False
            self.after(0, lambda: self.btn_start.configure(text="Schedule", fg_color="#27ae60"))
            self.after(0, lambda: self.label.configure(text="Execution time (HH:MM:SS):", text_color="#FFFFFF"))
